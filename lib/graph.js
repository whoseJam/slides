...
export class Graph {
    ...
    _update() {
        ...
        this._simulation = this._simulation
            .nodes(this._nodes)
            .force("link", d3.forceLink(this._links).id(data => data.id));
        console.log("UPDATE my SELF");
        
        this._simulation = this._simulation.on("tick", () => {
            this._links.forEach((link) => {
                link.line_handle.x1(link.source.x);
                link.line_handle.y1(link.source.y);
                link.line_handle.x2(link.target.x);
                link.line_handle.y2(link.target.y);
            });
            this._nodes.forEach((node) => {
    ...
    _check_node(id) {
        if (this._get_node(id) === null) {
            this._origin_nodes.push({
                id: id,
                value: new Text(this._group, String(id))
            });
        }
    }
