...
export class Element {
    ...
    color(color, animate = false) {
        create_handle(this.bound_, animate).fill({ opacity: 1 });
        create_handle(this.bound_, animate).fill({ color: color.main_color_ });
    }

    default_color(animate = false) {
        create_handle(this.bound_, animate).fill({ opacity: 0 });
    }
